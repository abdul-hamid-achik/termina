...
const LANE_ROUTES: Record<string, { radiant: string[]; dire: string[] }> = {
  top: {
    radiant: ['top-t3-rad', 'top-t2-rad', 'top-t1-rad', 'top-river', 'top-t1-dire', 'top-t2-dire', 'top-t3-dire', 'dire-base'],
    dire: ['top-t3-dire', 'top-t2-dire', 'top-t1-dire', 'top-river', 'top-t1-rad', 'top-t2-rad', 'top-t3-rad', 'radiant-base'],
  },
  mid: {
    radiant: ['mid-t3-rad', 'mid-t2-rad', 'mid-t1-rad', 'mid-river', 'mid-t1-dire', 'mid-t2-dire', 'mid-t3-dire', 'dire-base'],
    dire: ['mid-t3-dire', 'mid-t2-dire', 'mid-t1-dire', 'mid-river', 'mid-t1-rad', 'mid-t2-rad', 'mid-t3-rad', 'radiant-base'],
  },
  bot: {
    radiant: ['bot-t3-rad', 'bot-t2-rad', 'bot-t1-rad', 'bot-river', 'bot-t1-dire', 'bot-t2-dire', 'bot-t3-dire', 'dire-base'],
    dire: ['bot-t3-dire', 'bot-t2-dire', 'bot-t1-dire', 'bot-river', 'bot-t1-rad', 'bot-t2-rad', 'bot-t3-rad', 'radiant-base'],
  },
}
...
function getCreepAttack(type: CreepState['type']): number {
  switch (type) {
    case 'melee': return MELEE_CREEP_ATTACK
    case 'ranged': return RANGED_CREEP_ATTACK
    case 'siege': return SIEGE_CREEP_ATTACK
  }
}

/** Determine which lane a creep is on based on its zone. */
function getCreepLane(zone: string): string | null {
  if (zone.startsWith('top-') || zone === 'radiant-base' || zone === 'dire-base') return 'top'
...
function getEnemyCreepsInZone(creeps: CreepState[], creep: CreepState): CreepState[] {
  return creeps.filter(
    (c) => c.zone === creep.zone && c.team !== creep.team && c.hp > 0,
  )
}

/** Find enemy heroes in the same zone. */
function getEnemyHeroesInZone(players: Record<string, PlayerState>, zone: string, team: CreepState['team']): PlayerState[] {
  return Object.values(players).filter(
    (p) => p.zone === zone && p.team !== team && p.alive,
  )
}

/** Find enemy tower in the same zone. */
function getEnemyTowerInZone(towers: TowerState[], zone: string, team: CreepState['team']): TowerState | undefined {
