...
/** A single workflow run's event log (sleep + tick step, every 4s) grows
 *  without bound — Workflow DevKit's replay cost degrades past ~2,000
 *  events. Chain to a continuation well under that: ~4-6 events/tick ×
 *  250 ticks keeps a run's log in the low thousands, and 250 ticks is a full
 *  1000s (~16.7min) segment. */
export const CHAIN_EVERY_TICKS = 250

/**
 * Whether the CURRENT (durable) cycle number is a chain boundary. Keyed off
 * the cycle read back from Neon rather than this run's local loop index —
 * chaining must still land on the right boundary even if some of this run's
 * ticks were skipped (CAS losses), and a continuation reads its own cycle
 * fresh on start regardless.
 */
 ...
 *  exact deadline inside the step — the variant-3 pattern from tickSpike.ts,
 *  measured at ±2ms precision in the spike. */
async function gameTickStep(gameId: string, target: number): Promise<TickStepResult> {
  ...
  const result = await runOneTick(gameId, productionTickDeps())
  engineLog.debug('[gameTick] tick', { gameId, ...result, alignMs })
