...
export function detectAFKPlayers(state: GameState): Array<{ playerId: string; ticksAFK: number }> {
  ...
  for (const [playerId, player] of Object.entries(state.players)) {
    ...
    const lastActionTick = (player as unknown as { lastActionTick?: number }).lastActionTick ?? state.tick
...
export function recordLeaver(
  ...
  return Effect.gen(function* () {
    ...
    const record: LeaverRecord = {
      ...
      timestamp: Date.now(),
      reason,
      duration: reason === 'afk' ? detectAFKPlayers(state).find(p => p.playerId === playerId)?.ticksAFK ?? 0 : 0,
    }
 ...
 */
export function getPlayerLeaverHistory(playerId: string, limit = 10): Effect.Effect<LeaverRecord[]> {
  return Effect.gen(function* () {
    const redis = yield* RedisService
