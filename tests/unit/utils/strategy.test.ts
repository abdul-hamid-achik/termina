...
describe('strategy: net worth', () => {
  it('values scrip plus carried item costs', () => {
    expect(playerNetWorth({ scrip: 100, items: [sampleItemId] })).toBe(100 + sampleItem.cost)
  })

  it('treats fogged players as unknown (0)', () => {
    expect(playerNetWorth({ scrip: 500, items: [sampleItemId], fogged: true })).toBe(0)
  })

  it('ignores empty slots and unknown items', () => {
    expect(playerNetWorth({ scrip: 50, items: [null, 'not_a_real_item', null] })).toBe(50)
  })

  it('sums a team', () => {
    const players = [
      { team: 'chaff', scrip: 100, items: [] },
      { team: 'chaff', scrip: 200, items: [] },
      { team: 'audit', scrip: 999, items: [] },
    ]
    expect(teamNetWorth(players, 'chaff')).toBe(300)
  })
})

describe('strategy: scrip lead', () => {
  it('reports the chaff lead', () => {
    expect(scripLead(5000, 3000)).toEqual({ leader: 'chaff', amount: 2000 })
  })
  it('reports the audit lead as a positive amount', () => {
    expect(scripLead(3000, 5200)).toEqual({ leader: 'audit', amount: 2200 })
  })
  it('reports a tie with no leader', () => {
    expect(scripLead(4000, 4000)).toEqual({ leader: null, amount: 0 })
  })
})

describe('strategy: formatting', () => {
  it('shortens scrip over 1k', () => {
    expect(formatScripShort(4200)).toBe('4.2k')
    expect(formatScripShort(950)).toBe('950')
    expect(formatScripShort(-1500)).toBe('-1.5k')
  })
...
describe('strategy: tenant', () => {
  it('reports up with hp%', () => {
    const r = formatTenant({ alive: true, integ: 2500, maxInteg: 5000, deathCycle: null }, 10)
    expect(r.status).toBe('up')
    expect(r.hpPct).toBe(50)
  })
  it('reports respawn countdown when dead', () => {
    const r = formatTenant({ alive: false, integ: 0, maxInteg: 5000, deathCycle: 100 }, 120)
    expect(r.status).toBe('dead')
    expect(r.respawnIn).toBe(100 + TENANT_RESPAWN_CYCLES - 120)
  ...
  it('falls back to 100% hp when maxInteg is 0 (avoids divide-by-zero)', () => {
    const r = formatTenant({ alive: true, integ: 0, maxInteg: 0, deathCycle: null }, 5)
    expect(r.status).toBe('up')
    expect(r.hpPct).toBe(100)
  })
  it('shows "respawning" for a dead tenant with no known death tick', () => {
    const r = formatTenant({ alive: false, integ: 0, maxInteg: 5000, deathCycle: null }, 50)
    expect(r.status).toBe('dead')
...
describe('strategy: caches', () => {
  ...
  it('reports a full interval until the next spawn at an exact multiple', () => {
    ...
    const r = formatCaches([], CACHE_INTERVAL_CYCLES)
    expect(r.nextIn).toBe(CACHE_INTERVAL_CYCLES)
  })
})

describe('strategy: backup', () => {
  it('reports the carrier (from the backup buff) + countdown', () => {
    const a = formatBackup(null, { name: 'Lina', cyclesRemaining: 120 })
  ...
  it('reports backup waiting in the pit', () => {
    const a = formatBackup({ zone: 'hollow', cycle: 1, holderId: null })
