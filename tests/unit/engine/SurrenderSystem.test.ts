...
describe('SurrenderSystem', () => {
  ...
  describe('voteSurrender', () => {
    it('rejects votes before SURRENDER_MIN_CYCLE', () => {
      const state = makeGameState({ cycle: 0 })
    ...
    it('records the vote in the returned state', () => {
      const state = makeGameState({ cycle: SURRENDER_MIN_CYCLE })
      const result = voteSurrender(state, 'r1')
    ...
    it('accepts a vote from a dead player', () => {
      const base = makeGameState({ cycle: SURRENDER_MIN_CYCLE })
      const state = {
        ...base,
  ...
  describe('solo-vs-bots electorate', () => {
    ...
    it('a dead lone human concedes from the death overlay', () => {
      ...
      const result = Effect.runSync(processCycle('surr-solo-dead', dead))
      expect(result.state.phase).toBe('ended')
      expect(result.state.winner).toBe('audit')
    })

    it('ends the game when the lone human concedes via processCycle', () => {
      submitAction('surr-solo', 'human', { type: 'surrender', vote: 'yes' })
      const result = Effect.runSync(processCycle('surr-solo', soloWithBots()))
      expect(result.state.phase).toBe('ended')
      expect(result.state.winner).toBe('audit')
  ...
  describe('surrender via processCycle', () => {
    it('persists votes across ticks', () => {
      const state = makeGameState({ cycle: SURRENDER_MIN_CYCLE })
      submitAction('surr-1', 'r1', { type: 'surrender', vote: 'yes' })
      const result = Effect.runSync(processCycle('surr-1', state))
      expect(result.state.surrenderVotes.chaff.has('r1')).toBe(true)
    ...
    it('ends the game with the opposing team as winner when vote passes', () => {
      ...
      const mid = Effect.runSync(processCycle('surr-2', state))
      submitAction('surr-2', 'r2', { type: 'surrender', vote: 'yes' })
      const result = Effect.runSync(processCycle('surr-2', mid.state))
    ...
    it('a no vote retracts a previous yes vote', () => {
      ...
      const mid = Effect.runSync(processCycle('surr-3', state))
      expect(mid.state.surrenderVotes.chaff.has('r1')).toBe(true)

      submitAction('surr-3', 'r1', { type: 'surrender', vote: 'no' })
      const result = Effect.runSync(processCycle('surr-3', mid.state))
    ...
    it('emits a surrender_vote event for player feedback', () => {
      ...
      const result = Effect.runSync(processCycle('surr-4', state))
    ...
    it('rejects votes before the minimum tick with feedback', () => {
      ...
      const result = Effect.runSync(processCycle('surr-5', state))
      expect(result.state.surrenderVotes.chaff.size).toBe(0)
  ...
  describe('canSurrender', () => {
    it('is too early before SURRENDER_MIN_CYCLE', () => {
      const res = canSurrender(makeGameState({ cycle: 0 }), 'chaff')
    ...
    it('allows a team with humans past the minimum tick', () => {
      expect(canSurrender(makeGameState({ cycle: SURRENDER_MIN_CYCLE }), 'chaff').can).toBe(true)
    })
  })
